...
public class Builder {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println(
					String.format("Can't load Processing code: %s",
					PicodeSettings.getDefaultSketchPath()));
		} catch (SketchException se) {
			System.err.println(
					ProcessingIntegration.getErrorString(sketch, se));
		}
	}
	...
	HashMap<String, ArrayList<Library>> importToLibraryTable;
	...
	public synchronized void run() throws SketchException {
	 ...
	 */
	SketchException placeException(String message, String dotJavaFilename,
			int dotJavaLine) {
		int codeIndex = 0; // -1;
		int codeLine = -1;
		...
		// }
		// return new SketchException(message, codeIndex, codeLine);
		return new SketchException(message, codeIndex, codeLine, -1, false); // changed
																				// for
																				// 0194
																				// for
																				// compile
	...
	public ArrayList<Library> getCoreLibraries() {
		if (coreLibraries == null) {
			...
			coreLibraries
					.add(new Library(Base.getContentFile("lib/core"), null));
			coreLibraries.add(new Library(Base.getContentFile("lib/phybots"),
					null));
	...
	public Library getLibrary(String pkgName) throws SketchException {
		...
		if (libraries == null) {
			...
			String secondary = "The import " + pkgName
					+ " points to multiple libraries:<br>";
			for (Library library : libraries) {
				String location = library.getPath();
				if (location.startsWith(PicodeSettings.getLibrariesFolder()
						.getAbsolutePath())) {
					location = "part of Processing";
				}
				secondary += "<b>" + library.getName() + "</b> (" + location
						+ ")<br>";
			}
			secondary += "Extra libraries need to be removed before this sketch can be used.";
			Base.showWarningTiered("Duplicate Library Problem", primary,
					secondary, null);
			throw new SketchException("Duplicate libraries found for "
					+ pkgName + ".");
	...
	public ClassLoader getClassLoader() {
		return this.getClass().getClassLoader(); // Use the default class
													// loader.
	}
