...
public class EventThread extends Thread {
  ...
  public EventThread(Launcher launcher, PrintWriter writer) {
    super("event-handler");
   ...
   */
  public void run() { 
    EventQueue queue = launcher.vm.eventQueue();
    while (connected) {
      try {
  ...
  public void setEventRequests(boolean watchFields) {
    EventRequestManager mgr = launcher.vm.eventRequestManager();
...
//    System.out.println(o.getClass());
    ...
    for (Iterator it = fields.iterator(); it.hasNext(); ) {
      ...
      for (int i=0; i<launcher.excludes.length; ++i) {
        req.addClassExclusionFilter(launcher.excludes[i]);
      }
