...
import jp.digitalmuseum.roboko.ui.editor.DocumentManager;
...
public abstract class Pose implements Cloneable {
	...
	public static Pose load(String name) throws IOException {
		...
		BufferedReader reader = new BufferedReader(new FileReader(new File(
				RobokoSettings.getPoseFolderPath(), getDataFileName(name))));
		...
		pose.setPhoto(ImageIO.read(new File(
				RobokoSettings.getPoseFolderPath(), photoFileName)));
		return pose;
	}

	public void delete() {
		File dataFile = new File(RobokoSettings.getPoseFolderPath(), getDataFileName());
		if (dataFile.exists()) {
			dataFile.delete();
		}
		File photoFile = new File(RobokoSettings.getPoseFolderPath(), getPhotoFileName());
		if (photoFile.exists()) {
			photoFile.delete();
		}
	}
	...
	public void save() throws IOException {
		...
		if (getPhoto() != null) {
			ImageIO.write(getPhoto(), "JPEG", new File(
					RobokoSettings.getPoseFolderPath(), getPhotoFileName()));
		}
	}
	...
	public void rename(String name) {
		...
		if (dataFile.exists()) {
			dataFile.renameTo(
					new File(
							RobokoSettings.getPoseFolderPath(),
