	...
	Class<? extends Pose> getPoseClass() {
		try {
			return (Class<? extends Pose>)
					Class.forName(String.format("jp.digitalmuseum.roboko.core.internal.%sPose", this.toString()));
		} catch (ClassNotFoundException e) {
			return null;
		}
	}
	...
	Class<? extends MotorManager> getMotorManagerClass() {
		try {
			return (Class<? extends MotorManager>)
					Class.forName(String.format("jp.digitalmuseum.roboko.core.internal.%sMotorManager", this.toString()));
		} catch (ClassNotFoundException e) {
			return null;
		}
	}

	public MotorManager newMotorManagerInstance(RobokoMain robokoMain, Robot robot) {
		try {
			return getMotorManagerClass()
					.getConstructor(RobokoMain.class, Robot.class)
